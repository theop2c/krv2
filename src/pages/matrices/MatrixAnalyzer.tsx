...
export function MatrixAnalyzer() {
  ...
  const handleFileSelect = async (file: FichierTeleverse) => {
    ...
    // Analyze document for each question
    try {
  ...
  const handleSave = async () => {
    ...
    const data = {
      ...
      responses: Object.entries(finalResponses).map(([questionId, response]) => ({
        questionId,
        response
      }))
    };
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      await saveAnalysis(
        ...
        analysisData.fileId,
        Object.fromEntries(analysisData.responses.map(r => [r.questionId, r.response])),
        analysisData.name
      );
