...
interface MatrixState {
  ...
  saveAnalysis: (matrixId: string, fileId: string, responses: Record<string, string>, name: string) => Promise<void>;
...
export const useMatrixStore = create<MatrixState>((set, get) => ({
  ...
  saveAnalysis: async (matrixId: string, fileId: string, responses: Record<string, string>, name: string) => {
    try {
      set({ loading: true, error: null });
      const analysis = {
        matrixId,
      ...
      await addDoc(collection(db, 'matrix_analyses'), analysis);
      set({ loading: false });
    } catch (error) {
  ...
  fetchAnalyses: async (userId: string) => {
    try {
      set({ loading: true, error: null });
      const analysesRef = collection(db, 'matrix_analyses');
